...
1. Voice/speech-to-text errors (garbled words or wrong words from misrecognition) — NEVER penalize. Grade intended meaning only.
2. This is spoken conversation, not a written exam. Grade spoken fluency.
3. Real-world knowledge, cultural depth, practical nuance = high comprehension score.
...
// ── Per-rank rubrics ──────────────────────────────────────────────────────────
const BEGINNER_RUBRIC = `Be extremely generous. Reward any genuine attempt in French.
...
Grade scale:
- A: Any French used, understood the topic, made an effort
- B: Mostly English but sprinkled French, showed comprehension
- C: Barely tried but attempted something in French
- D: Almost entirely English, minimal effort
- F: Zero French, refused to engage`;
...
const RANK_RUBRICS: Record<string, string> = {
  ...
  advanced: `You are grading an early intermediate French student.
They can form coherent sentences and communicate a clear idea.
Expect correct basic sentence structure, familiar topics, and mostly present tense.
Think: a student who can manage everyday French but is not yet comfortable with complex grammar.

Standards:
- Can form one complete, grammatically reasonable sentence per response
- Vocabulary covers food, family, daily life, plans, and simple opinions
- Grammar: basic word order, present tense, gender/number agreement attempts, simple connectors like parce que, mais, et
...
Tutor: Qu'est-ce que tu aimes faire le week-end ?
User: J'aime jouer au basket parce que c'est amusant, et je joue avec mes amis.
→ Clear sentence, reason given, simple vocab, makes sense. A grade.`,

  pro: `You are grading an upper intermediate French student.
...
Think: someone who studied French for several years and uses it occasionally.

Standards:
- 2-3 sentences minimum per response
- No English mixing tolerated
- Functional everyday vocabulary beyond survival phrases
- Can express opinions, describe situations, give reasons
- Uses present tense reliably and attempts past or future when needed
- Grammar doesn't have to be perfect but meaning must be clear`,

  iron: `You are grading a strong intermediate French student.
They can hold a real conversation on familiar topics.
3 solid sentences with some vocabulary depth expected.
Think: can communicate comfortably and sustain a topic.

Standards:
- 3+ sentences, coherent and on-topic
- Some varied vocabulary used naturally
- No English mixing
- Can sustain a topic, give opinions with reasoning
- Handles past and future tenses with understandable control
- Minor grammar errors acceptable if meaning is clear

Example A-grade at Iron:
Tutor: Quelles sont les différences entre la vie en ville et la vie à la campagne ?
User: La vie en ville est plus pratique parce qu'il y a beaucoup de magasins et de transports, mais elle est aussi bruyante. La campagne est plus calme et l'air est meilleur, pourtant il est parfois plus difficile de trouver du travail. Je préfère la ville parce que j'aime l'énergie et les activités.
→ 3 sentences, comparison structure, varied vocabulary, clear. A grade.`,

  gold: `Same as Iron standard but slightly stricter.
Expect richer vocabulary and smoother flow.
Expect fluent multi-sentence responses with B2 vocabulary range.
Connectors should appear naturally, such as non seulement...mais aussi, bien que, cependant, pourtant.
...
Standards:
- C1 vocabulary appearing naturally
- Natural sentence variety, including subordinate clauses
- Connectors and discourse markers used correctly
- Can discuss feelings, comparisons, hypotheticals
- Appropriate idioms and register awareness
- No English. No formulaic repetition.`,

  ethereal: `You are grading a native speaker or near-native speaker.
This is someone who grew up speaking French or has lived in a French-speaking environment for years. Think: 10+ year speaker.
...
- Must feel like talking to a well-read, articulate native speaker`,
  ...
  eternal: `You are grading a scholar of the French language.
Think: a French literature professor, a historical linguist,
...
- Educated native diction and literary register
...
Grade the student on their French language performance at the ${effectiveRank} rank standard. Return ONLY valid JSON matching this exact structure:
{
...
- MEDIUM mode: evaluate French output and appropriate use of English clarification.
- HARD mode: evaluate sustained French-only communication.`;
