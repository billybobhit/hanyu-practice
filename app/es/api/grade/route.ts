...
// ── Per-rank rubrics ──────────────────────────────────────────────────────────
const BEGINNER_RUBRIC = `Be extremely generous. Reward any genuine attempt in Spanish.
...
Grade scale:
- A: Any Spanish used, understood the topic, made an effort
- B: Mostly English but sprinkled Spanish, showed comprehension
- C: Barely tried but attempted something in Spanish
- D: Almost entirely English, minimal effort
- F: Zero Spanish, refused to engage`;
...
const RANK_RUBRICS: Record<string, string> = {
  ...
  advanced: `You are grading an early intermediate Spanish student.
...
Think: a student who can manage everyday Spanish but is not yet comfortable with complex grammar.
...
- Grammar: basic word order, present tense, gender/number agreement attempts, simple connectors like porque, pero, y
...
Tutor: ¿Qué te gusta hacer los fines de semana?
User: Me gusta jugar al baloncesto porque es divertido, y juego con mis amigos.
→ Clear sentence, reason given, simple vocab, makes sense. A grade.`,

  pro: `You are grading an upper intermediate Spanish student.
...
Think: someone who studied Spanish for several years and uses it occasionally.
...
- Grammar doesn't have to be perfect but meaning must be clear`,
  ...
  iron: `You are grading a strong intermediate Spanish student.
...
Tutor: ¿Cuáles son las diferencias entre la vida en la ciudad y la vida en el campo?
User: La vida en la ciudad es más práctica porque hay muchas tiendas y transporte, pero también es ruidosa. El campo es más tranquilo y el aire es mejor, aunque a veces es más difícil encontrar trabajo. Prefiero la ciudad porque me gusta la energía y las actividades.
...
Connectors should appear naturally, such as no solo...sino también, aunque, sin embargo, por lo tanto, a pesar de.
...
This is someone who grew up speaking Spanish or has lived in a Spanish-speaking environment for years. Think: 10+ year speaker.
...
Think: Spanish-speaking person who went to university, reads books,
...
- Must feel like talking to a well-read, articulate native speaker`,
  ...
  eternal: `You are grading a scholar of the Spanish language.
Think: a Spanish literature professor, a historical linguist,
...
Grade the student on their Spanish language performance at the ${effectiveRank} rank standard. Return ONLY valid JSON matching this exact structure:
{
...
- MEDIUM mode: evaluate Spanish output and appropriate use of English clarification.
- HARD mode: evaluate sustained Spanish-only communication.`;
