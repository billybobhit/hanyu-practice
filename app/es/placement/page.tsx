...
export default function EsPlacementPage() {
  ...
  const speakText = useCallback((text: string) => {
    ...
    const preferredVoice = voices.find((v) => v.lang.includes("es"));
  ...
  useEffect(() => {
    async function init() {
      ...
      const placementStatus = await getPlacementStatus(supabase, user.id, "es");
    ...
    try {
      ...
      const endedSession: Session = {
        ...
        materialTitle: isAdvancedPlacement
          ? "Advanced Placement Assessment · Spanish"
          : "Placement Assessment · Spanish",
        materialContent: "",
        difficulty: "hard",
        messages,
        startTime: Date.now(),
      ...
      <header className="glass border-b border-ink-600 px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1">
          ...
          <p className="text-cream-600 text-xs">
            Spanish · {userTurns}/{requiredTurns} turns
          </p>
        </div>
        <button
          onClick={() => {
