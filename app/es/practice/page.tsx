...
export default function PracticePage() {
  ...
  const handleDevCommand = () => {
    ...
    if (grade) {
      ...
      setTimeout(() => {
        setDevMode(false);
        router.push(`/es/results?preview=${grade}`);
      }, 700);
    } else {
      setDevMessages((prev) => [
        ...prev,
  ...
  const speakText = useCallback(
    (text: string) => {
      ...
      utterance.lang = session?.difficulty === "easy" ? "en-US" : "es-ES";
      utterance.rate = 0.85;
  ...
  const streamAssistant = useCallback(
    async (currentSession: Session, messages: Message[]) => {
      ...
      try {
        const response = await fetch("/es/api/chat", {
          method: "POST",
  ...
  const endSession = async () => {
    ...
    try {
      const previousProgress = getProgressFromSessions(
        getSessions().filter((s) => s.id !== session.id && s.languageCode === "es")
      );

      const gradeAbort = new AbortController();
      gradeTimeout = setTimeout(() => gradeAbort.abort(), GRADE_TIMEOUT_MS);
      const response = await fetch("/es/api/grade", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    <div className="h-screen flex flex-col bg-ink-900 pt-16">
      ...
      {gradingError && (
        <div className="bg-ink-700/80 border-t border-ink-500 px-4 py-3 shrink-0 flex items-center gap-3">
          ...
          <button
            onClick={() => router.push("/es/results")}
